...
const metadataFields = [
  { tag: "(0010,0010)", name: "Patient Name", before: "John Michael Smith", after: "ANONYMIZED", redacted: true },
  { tag: "(0010,0020)", name: "Patient ID", before: "MRN-2024-88421", after: "ANONYMIZED", redacted: true },
  { tag: "(0010,0030)", name: "Date of Birth", before: "19850315", after: "ANONYMIZED", redacted: true },
  { tag: "(0010,0040)", name: "Patient Sex", before: "M", after: "M", redacted: false },
  { tag: "(0008,0020)", name: "Study Date", before: "20240115", after: "ANONYMIZED", redacted: true },
  { tag: "(0008,0050)", name: "Accession Number", before: "ACC-2024-00142", after: "ANONYMIZED", redacted: true },
  { tag: "(0008,0060)", name: "Modality", before: "CT", after: "CT", redacted: false },
  { tag: "(0008,0070)", name: "Manufacturer", before: "SIEMENS", after: "SIEMENS", redacted: false },
  { tag: "(0008,1030)", name: "Study Description", before: "CT CHEST W/O CONTRAST", after: "CT CHEST W/O CONTRAST", redacted: false },
  { tag: "(0008,0090)", name: "Referring Physician", before: "Dr. Sarah Williams", after: "ANONYMIZED", redacted: true },
  { tag: "(0008,1070)", name: "Operators' Name", before: "TECH_JOHNSON_R", after: "ANONYMIZED", redacted: true },
  { tag: "(0010,1010)", name: "Patient Age", before: "039Y", after: "ANONYMIZED", redacted: true },
];
...
const Preview = () => {
  return (
    ...
    >
      <div>
        <h1 className="text-2xl font-bold text-foreground">Before / After Preview</h1>
        <p className="text-muted-foreground text-sm mt-1">Compare original and de-identified data side by side</p>
      </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary" className="bg-success/10 text-success border-0 font-semibold">
              {metadataFields.filter((f) => f.redacted).length} PHI Fields Redacted
            </Badge>
          </div>
        </CardHeader>
        <CardContent>
          <div className="rounded-xl border border-border overflow-hidden">
            ...
            <div className="grid grid-cols-12 gap-0 bg-muted/50 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              ...
              <div className="col-span-3 px-4 py-2.5 flex items-center gap-1.5"><Eye className="w-3.5 h-3.5" /> Original</div>
              <div className="col-span-3 px-4 py-2.5 flex items-center gap-1.5"><EyeOff className="w-3.5 h-3.5" /> De-identified</div>
            ...
            {metadataFields.map((field, i) => (
              ...
              >
                <div className="col-span-2 px-4 py-2.5 font-mono text-xs text-muted-foreground">{field.tag}</div>
                <div className="col-span-2 px-4 py-2.5 text-foreground font-medium">{field.name}</div>
                <div className={`col-span-3 px-4 py-2.5 font-mono text-xs ${field.redacted ? "text-destructive line-through" : "text-foreground"}`}>
                  {field.before}
                </div>
                <div className={`col-span-3 px-4 py-2.5 font-mono text-xs ${field.redacted ? "text-success font-semibold" : "text-foreground"}`}>
                  {field.after}
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-semibold text-foreground">De-identified</span>
              </div>
